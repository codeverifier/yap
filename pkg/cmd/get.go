...
func (o *GetOptions) Command() *cobra.Command {
	var cmd = &cobra.Command{
		Use:   "get [type] [name]",
		Short: "Read currently running clusters",
...
`,
		Example: "  yap get\n" +
			"  yap get cluster microk8s -o yaml\n" +
			"  yap get cluster kind-kind -o template --template '{{.status.localRegistryHosting.host}}'\n",
		Run:  o.Run,
...
func (o *GetOptions) Run(cmd *cobra.Command, args []string) {
	...
	default:
		_, _ = fmt.Fprintf(o.ErrOut, "Unrecognized type: %s. Possible value: cluster.\n", t)
		os.Exit(1)
	}
