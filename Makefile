...
install:
	CGO_ENABLED=0 go install ./cmd/yap
...
fmt:
	goimports -w -l -local github.com/pseudonator/yap cmd/ internal/ pkg/
...
publish-ci-image:
	...
	docker buildx build --builder=$(BUILDER) --pull --platform=linux/amd64,linux/arm64 --push -t docker/yap-ci -f .circleci/Dockerfile .
